...
import { useGetLecByClassMutation, useLazyGetLecsQuery } from '../../redux/api/lecsApiSlice';
...
export default function VideoListView() {
  ...
  const [getLecs]=useGetLecByClassMutation();
  ...
  useEffect(()=>{
    ...
    const getLecsnow=async ()=>{
        try{
            ...
            const res=await getLecs(lecid).unwrap();
            setVideos(res.lec);
  ...
  const handleVideoClick = (path,path2) => {
    ...
    dispatch(setLecId(path));
    dispatch(setCredentials(path2));
