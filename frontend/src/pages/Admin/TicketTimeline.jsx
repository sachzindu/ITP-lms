...
import { useLazyGetTicketDetailsQuery } from '../../redux/api/supportApiSlice';
...
const TicketTimeline = () => {
  ...
  const { id } = useParams();
  const [getTicket]=useLazyGetTicketDetailsQuery();
  ...
  useEffect(() => {
   ...
   const getTicketsNow= async ()=>{
    setLoading(true);
    try{
    const response=await getTicket(id).unwrap();
    if(response){
      setLoading(false);
      setTicket(response);
    }}
